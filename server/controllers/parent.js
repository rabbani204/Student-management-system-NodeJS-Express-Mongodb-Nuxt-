...
exports.getdetails = (req, res) => {
    Parent.findById(req.params.id, (err, ins) => {
        if (err) {
            res.json({
                error: err
            })
...
exports.getBycontact = (req, res) => {
    Parent.findOne({ parent_contact: req.params.id }, (err, student) => {
        if (err) {
            res.json({
                success: false,
                error: err
...
exports.registerParent = (req, res) => {
    console.log(req.body, 'here is parent body')
    ...
    // Parent.find({ parent_contract: req.body.institution_contract }, (err, ins) => {
    Parent.find({ parent_contact: req.body.parent_contact }, (err, parent) => {
        console.log(parent, 'fount parent', err)
        if (parent.length) {
            res.json({
                error: err,
                message: "Parent already exis!"
            })
        }
        else {
            bcrypt.hash(req.body.password, 10, (err, hash) => {
                if (err) {
                    ...
                    const output = `
							...
							<ul>  
							<li>Username: ${req.body.parent_name}</li>
...
exports.registerSubmitConfirmation = (req, res) => {
    Parent.find({ parent_contact: req.body.parent_contact }, (err, parent) => {
        ...
        if (err) {
            ...
            if (parent[0].sendKey == req.body.confirmation_code) {
                parent[0].sendKey = 'completed'

                parent[0].save().then(docs => {
                    res.json({
                        success: true,
                        data: docs
                    })
...
exports.login = (req, res) => {
    ...
    Parent.find({ parent_contact: req.body.parent_contact }, (err, parent) => {
        console.log(parent, err, 'user form contr')
        if (!parent.length) {
            return res.json({
                message: "Auth Failed",
                success: false,
            ...
            bcrypt.compare(req.body.password, parent[0].password, (err, result) => {
                ...
                if (result) {
                    const jwtTkoen = token.sign({
                        id: parent[0].id,
                        role: parent[0].role,
                        parent_contact: parent[0].parent_contact,
                        uniqueId: parent[0].uniqueId
...
exports.recoveryPassword = (req, res) => {
    ...
    Parent.find({ parent_contact: req.body.institution_contract }, (err, ins) => {
...
exports.recoveryPassVerificationSubmit = (req, res) => {
    Parent.find({ parent_contact: req.body.parent_contact }, (err, ins) => {
